...
module.exports.getNewsList = async function (page) {
  const news = await axios.get(
    process.env.ServerURL + "/news?page=" + page * 1
  );
  const res = [];
  for (const item of news.data) {
    ...
    !text
      ? (text = "본문이 사진으로 이루어져있거나 내용이 없는 기사입니다.")
...
module.exports.getNewsRecent = async function () {
  const cnt = await axios.get(process.env.ServerURL + "/news/cnt");
  const newsId = [];
  let lowId;
  if (cnt.data > 5000) {
    lowId = cnt.data - 5000;
  ...
  if (newsId.length === 5) {
    const news = await axios.get(
      process.env.ServerURL +
        `/news/recent?o=${newsId[0]}&t=${newsId[1]}&th=${newsId[2]}&f=${newsId[3]}&fi=${newsId[4]}`
    );
    const res = [];
    for (const item of news.data) {
      const $ = await cheerio.load(item.contents);
    ...
    const news = await axios.get(
      process.env.ServerURL + `/news/recent?o=1&t=2&th=3&f=4&fi=5`
    );
    for (const item of news.data) {
      const $ = await cheerio.load(item.contents);
      let img = await $("img").attr("src");
...
module.exports.cntNews = async function () {
  const count = await axios.get(process.env.ServerURL + "/news/cnt");
  return count.data;
};

module.exports.getPressNews = async function (name, page) {
  const news = await axios.get(
    process.env.ServerURL + "/press/" + encodeURI(name) + "/news?page=" + page
  );
  const res = [];
  for (const item of news.data) {
    const $ = await cheerio.load(item.contents);
...
module.exports.cntPressNews = async function (name) {
  const cnt = await axios
    .get(process.env.ServerURL + "/press/" + encodeURI(name) + "/cnt")
...
module.exports.getTopicNews = async function (name, page) {
  const news = await axios.get(
    process.env.ServerURL + "/topic/" + encodeURI(name) + "/news?page=" + page
  );
  const res = [];
  for (const item of news.data) {
    const $ = await cheerio.load(item.contents);
...
module.exports.cntTopicNews = async function (name) {
  const cnt = await axios
    .get(process.env.ServerURL + "/topic/" + encodeURI(name) + "/cnt")
    .catch((err) => console.log(err));
  return cnt.data;
};
...
module.exports.getMyNewsByPress = async function (id, page) {
  const news = await axios.get(
    process.env.ServerURL + "/main/press?id=" + id + "&page=" + page
  );
  const res = [];
  for (const item of news.data) {
    const $ = await cheerio.load(item.contents);
...
module.exports.getMyNewsTopic = async function (id, page) {
  const news = await axios.get(
    process.env.ServerURL + "/main/topic?id=" + id + "&page=" + page
  );
  const res = [];
  for (const item of news.data) {
    const $ = await cheerio.load(item.contents);
...
module.exports.getNewsLe = async function (id, uId) {
  const newsWrapper = await axios.get(process.env.ServerURL + "/news/" + id);
  ...
  if (minus) {
    news.contents = news.contents.replace(
      minus,
      `<a class="viedoLink" target="_blank" href="${news.href}">해당 기사는 동영상 뉴스입니다.<br>동영상은 해당 링크에서 시청하실 수 있습니다. </a>`
    );
  }
  news.contents = news.contents.slice(0, news.contents.indexOf("<a href") - 4);
  news.contents = news.contents.slice(
    0,
    news.contents.indexOf("<a target") - 4
  );
...
async function checkSave(uId, nId) {
  const res = await axios.get(
    process.env.ServerURL + `/news/save/chk?uId=${uId}&nId=${nId}`
  );
  return res.data;
}
...
module.exports.getNewsReply = async (id) => {
  const reply = await axios.get(
    process.env.ServerURL + "/news/" + id + "/reply" + "?page=0"
  );
  const res = [];
...
module.exports.getNewsReplyCnt = async (id) => {
  const reply = await axios.get(
    process.env.ServerURL + "/news/" + id + "/reply/cnt"
  );
...
module.exports.saveNewsReply = async (newsId, userId, contents) => {
  ...
  await axios({
    ...
    url: process.env.ServerURL + "/news/" + newsId + "/reply",
    data: data,
  }).catch((err) => console.log(err));
};

module.exports.getReplyContents = async (id) => {
  const contents = await axios.get(process.env.ServerURL + "/news/reply/" + id);
...
module.exports.updateReplyContents = async (id, contents) => {
  ...
  await axios({
    ...
    url: process.env.ServerURL + "/news/reply/" + id,
    data: JSON.stringify(data),
  }).catch((err) => console.log(err));
  const reply = await axios.get(process.env.ServerURL + "/news/reply/" + id);
  return reply.data;
};

module.exports.deleteReply = async (id) => {
  await axios.delete(process.env.ServerURL + "/news/reply/" + id);
};

module.exports.getAddReply = async (id, page) => {
  const reply = await axios.get(
    process.env.ServerURL + "/news/" + id + "/reply" + "?page=" + page
  );
  const res = [];
  for (const item of reply.data) {
    if (!item.createdDate[5]) item.createdDate[5] = "00";
...
module.exports.saveNews = async (uId, nId) => {
  ...
  const saveNews = await axios({
    ...
    url: process.env.ServerURL + "/news/save",
    data: data,
  }).catch((err) => console.log(err));
...
module.exports.deleteNews = async (id) => {
  const del = await axios.delete(process.env.ServerURL + "/news/save/" + id);
  return del.data;
};

module.exports.cntSaveNews = async (id) => {
  const cnt = await axios.get(
    process.env.ServerURL + "/news/save/cnt?id=" + id
  );
  return cnt.data;
};
...
module.exports.getSaveNews = async (id, page) => {
  const news = await axios
    .get(process.env.ServerURL + "/news/save/get?page=" + page + "&id=" + id)
    .catch((err) => console.log(err));
  const res = [];
  for (const item of news.data) {
    const $ = await cheerio.load(item.contents);
