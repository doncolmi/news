...
const headers = {
  "User-Agent":
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4097.0 Safari/537.36",
};

const ax = (url) =>
  axios.get(url, { responseEncoding: "binary", responseType: "arraybuffer" });

const updateNews = async () => {
  for (const item of topic) {
    await ax(
      `https://news.naver.com/main/list.nhn?mode=LSD&mid=sec&sid1=${item.url}&listType=title`
    ).then(async (htmlDoc) => {
      let ress = [];
      ...
      await updatePress(new Set([...pressList])).catch((err) =>
        console.log(err)
      );
...
const cntHref = async (href) => {
  const res = await axios.get(
    process.env.ServerURL + "/news/chk?href=" + href.replace(/&/g, "%26")
  );
  return res.data;
};
...
const getContents = async (url) => {
  ...
  await ax(url).then((htmlDoc) => {
    ...
    if (content === null || content === undefined) {
      content = col.find("div#newsEndContents").html();
    }
